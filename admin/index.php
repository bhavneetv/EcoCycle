...
<head>
    ...
    <title>EcoCycle Admin Dashboard</title>
...
<body class="min-h-screen text-white">
    ...
    <div id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 glassmorphism-sidebar sidebar-transition transform -translate-x-full lg:translate-x-0">
        ...
        <nav class="mt-6">
            <div class="px-4 space-y-2">
                <a href="#" disabled class="nav-item active flex items-center space-x-3 px-4 py-3 text-gray-300 rounded-xl hover:text-green-400 transition-all" data-section="dashboard">
        ...
        <main class="p-4 lg:p-8">
            <div id="dashboard-section">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <div class="stat-card text-white p-6 rounded-2xl shadow-2xl floating-animation">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-white/80 text-sm">Total Users</p>
                                <p class="text-3xl font-bold mt-1 text-shadow"></p>
                                <p class="text-green-300 text-sm mt-2">
                                    <!-- <i class="fas fa-arrow-up mr-1"></i>+12% from last month -->
                                </p>
                            </div>
                            <div class="bg-white/20 p-3 rounded-xl shimmer">
                                <i class="fas fa-users text-2xl"></i>
                            </div>
                        </div>
                    </div>

                    <div class="stat-card-green text-white p-6 rounded-2xl shadow-2xl floating-animation" style="animation-delay: -1s">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-white/80 text-sm">Bottle Scans</p>
                                <p class="text-3xl font-bold mt-1 text-shadow"></p>
                    ...
                    <div class="stat-card-purple text-white p-6 rounded-2xl shadow-2xl floating-animation" style="animation-delay: -2s">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-white/80 text-sm">Active Recyclers</p>
                                <p class="text-3xl font-bold mt-1 text-shadow"></p>
                    ...
                    <div class="stat-card-orange text-white p-6 rounded-2xl shadow-2xl floating-animation" style="animation-delay: -3s">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-white/80 text-sm">Rewards Claimed</p>
                                <p class="text-3xl font-bold mt-1 text-shadow"></p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <!-- Recent Bottle Requests -->
                    <div class="glassmorphism-card rounded-2xl shadow-2xl p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-lg font-semibold text-white text-shadow">Recent Bottle Requests</h3>
                            <!-- <button class="text-green-400 hover:text-green-300 text-sm font-medium transition-colors">View All</button> -->
                        </div>
                        <div class="space-y-4">
                    ...
                    <div class="glassmorphism-card rounded-2xl shadow-2xl p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-lg font-semibold text-white text-shadow">Top Recyclers This Month</h3>
                            <!-- <button class="text-green-400 hover:text-green-300 text-sm font-medium transition-colors">View All</button> -->
                        </div>
                        <div class="space-y-4">
    ...
    <script>
        async function loadDashboard() {
            ...
            // Update stat cards
            document.querySelector("#dashboard-section .stat-card .text-3xl").innerText = data.total_users;
            document.querySelector(".stat-card-green .text-3xl").innerText = data.bottle_requests;
            document.querySelector(".stat-card-purple .text-3xl").innerText = data.active_recyclers;
            document.querySelector(".stat-card-orange .text-3xl").innerText = data.rewards_claimed;

            // Recent requests
            const recentContainer = document.querySelector("#dashboard-section .glassmorphism-card .space-y-4");
            recentContainer.innerHTML = "";
            data.recent_requests.forEach(req => {
                recentContainer.innerHTML += `
            <div class="flex items-center space-x-4 p-3 glassmorphism rounded-xl">
                <div class="w-12 h-12 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center glow-effect">
                    <i class="fas fa-user text-white"></i>
                </div>
                <div class="flex-1">
                    <p class="font-medium text-white">${req.full_name}</p>
                    <p class="text-sm text-gray-300">${req.totalBottles} bottles </p>
                </div>
                <div class="text-right">
                    <span class="bg-yellow-500/20 text-yellow-300 text-xs px-3 py-1 rounded-full border border-yellow-500/30">${req.status}</span>
                    <p class="text-xs text-gray-400 mt-1">${new Date(req.created_at).toLocaleString()}</p>
                </div>
            </div>`;
            });

            // Top recyclers
            const recyclerContainer = document.querySelector("#dashboard-section .glassmorphism-card:last-child .space-y-4");
            recyclerContainer.innerHTML = "";
            data.top_recyclers.forEach((rec, i) => {
                recyclerContainer.innerHTML += `
            <div class="flex items-center space-x-4 p-3 glassmorphism rounded-xl">
                <div class="w-8 h-8 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-full flex items-center justify-center glow-effect">
                    <span class="text-white font-bold text-sm">${i + 1}</span>
                </div>
                <div class="flex-1">
                    <p class="font-medium text-white">${rec.full_name}</p>
                    <p class="text-sm text-gray-300">${rec.total_bottles} bottles recycled</p>
                </div>
